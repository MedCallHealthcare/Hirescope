...
class ResultsView(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        self.export_menu = ctk.CTkOptionMenu(
            ...
            values=["CSV", "Excel (.xlsx)", "ZIP", "Make.com"],
    ...
    def update_results(self, jobs: list[dict], duplicates_removed: int = 0, positive_keywords_filtered: int = 0):
    ...
    def _trigger_export(self):
        ...
        if choice == "Select format":
            messagebox.showwarning("No Format Selected", "Please select an export format first.")
            return
    ...
    def _export_excel(self):
        ...
        # Generate default filename with format: scrape_jobs_MM-DD-YYYY_HH_MM_AM/PM.xlsx
        now = datetime.now()
        default_filename = now.strftime("scrape_jobs_%m-%d-%Y_%I_%M_%p.xlsx")
            ...
            try:
                with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
                    ...
                    for title, count in sorted(self.stats["job_titles_data"].items(), key=lambda x: x[1], reverse=True):
                        job_titles_data.append(['', '', title, count])
                    
                    # Combine both sections
                    max_rows = max(len(companies_data), len(job_titles_data))
                    summary_data = []
                    
                    for i in range(max_rows):
                        if i < len(companies_data) and i < len(job_titles_data):
                            # Both have data
                            summary_data.append(companies_data[i][:2] + job_titles_data[i][2:])
                        elif i < len(companies_data):
                            # Only companies have data
                            summary_data.append(companies_data[i])
                        else:
                            # Only job titles have data
                            summary_data.append(job_titles_data[i])
                ...
                # Success message
                print(f"✅ Successfully exported Excel file!")
                print(f"   Location: {file_path}")
                print(f"   Total jobs: {len(self.jobs_data)}")
